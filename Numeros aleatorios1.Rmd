...
funcionUnif <- function(n){
  ...
  m = 2^(31)-1 #módulo
  u = c() 
  x0 = floor(runif(1, min = 1, max = 200)) #semilla
  xi = (a*x0)%%m 
  x0 = xi 
  for(i in 1:n){
...
***1. EXPONENCIAL***
```{r}
exponencial <- function(n, lamb){  #n= numero de variables aleatorias y lambda es el parámetro de la exponencial
  u = funcionUnif(n) # n variables aleatorias uniformes
...
***2. UNIFORME CONTINUA***
```{r}
uniforme <-function(n, inicioInt, finalInt){ #se definen los parámetros,inicioint: inicio del intervalo, finalint: fin del intervalo
  u = funcionUnif(n) #se generan numeros aleatorios uniformes
...
```{r}
binomial <- function(m, n, p){ # m=numero de variables aleatorias, n y p parametros de la Binomial

  u = funcionUnif(m) #se generan numeros aleatorios uniformes
...
*Un examen tiene 10 preguntas y cada una tiene 3 opciones como respuesta, siendo solo una de ellas la correcta.Si un estudiante contesta al azar, la probabilidad de que tenga mas de 5 preguntas correctas es de 0.2334. ¿Cuantas veces ocurrira este evento en 7 observaciones?*
...
```{r}
uniforme <- function(muest, valores){ #muest=numero de uniformes, valores es el parámetro de la uniforme discreta
  u = funcionUnif(muest) #se crean  variables aleatorias uniformes
...
```{r}
pois <- function(muest, lamb){ #muest= número de  variables uniformes y lambda el parámetro de la Poisson 
  u = funcionUnif(muest) #se crean las variables aleatorias uniformes
...
```{r}
geo <- function(n, prob){  #n= numero de variables aleatorias geométricas y prob es el parámetro de la distribución geométrica
  u = funcionUnif(n) #se crean n variables aleatorias uniformes
...
binNeg <- function(n, exi, prob){ #n=número de variables aleatorias que distribuyen binomial negativa donde exi y prob son los parámetros 
  bin = c() #se crea un vector vacio para llenarlo con variables aleatorias que distribuyen binomial negativa
  for(i in 1:n){
    geometrica = (geo(exi,1-prob)-1) #La binomial negativa esta dada por la suma de r geométricas. Creamos un vector de geométricas con paramétros exitos y probabilidad (1-p)-1
...
*Se lanzan repetidas veces una moneda equilibrada y los dos resultados son cara o cruz; la proabilidad de obtener la tercera cruz en el quinto lanzamiento es de 0.1875 ¿Cuantas veces ocurrira en 10 lanzamientos?*
...
***BERNOULLI***
```{r}
bernoulli <-function(n, prob){ #parámetros de la distribución Bernoulli 
  u = funcionUnif(n)  #Se generan numeros aleatorios uniformes
